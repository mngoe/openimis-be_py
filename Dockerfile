...
WORKDIR /
RUN git clone https://github.com/mngoe/openimis-be-report-cs_py.git
WORKDIR /openimis-be-report-cs_py
RUN git checkout develop-cs
RUN git pull origin develop-cs
WORKDIR /
RUN git clone https://github.com/mngoe/openimis-be-claim_py.git
WORKDIR /openimis-be-claim_py
RUN git checkout develop-cs
RUN git pull origin develop-cs
...
RUN pip install -e ../openimis-be-report-cs_py
RUN pip install -e ../openimis-be-claim_py
ARG SENTRY_DSN
