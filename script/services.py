...
def create_master_data_export(user):
    queries = {
        ...
        "services": """SELECT "ServiceID", "ServiceUUID", "ServCategory", "ServCode", "ServName", "ServType", "ServPackageType", "manualPrice", "ServLevel", "ServPrice", "ServCareType", "ServFrequency", "ServPatCat", TO_CHAR("ValidityFrom", 'yyyy-MM-dd')ValidityFrom, TO_CHAR("ValidityTo", 'yyyy-MM-dd')ValidityTo, "AuditUserID" FROM "tblServices" WHERE "ValidityTo" IS NULL"""
        if connection.vendor == "postgresql" else
        """SELECT "ServiceID", "ServiceUUID", "ServCategory", "ServCode", "ServName", "ServType", "ServPackageType", "manualPrice", "ServLevel", "ServPrice", "ServCareType", "ServFrequency", "ServPatCat", FORMAT("ValidityFrom", 'yyyy-MM-dd')ValidityFrom, FORMAT("ValidityTo", 'yyyy-MM-dd')ValidityTo, "AuditUserID" FROM "tblServices" WHERE "ValidityTo" IS NULL""",

        "subservices": """SELECT "idSCP", "ServiceId", "ServiceLinked", "qty", "price", "status" FROM "tblServiceContainedPackage" """,

        "items": """SELECT "ItemID", "ItemUUID", "ItemCode", "ItemName", "ItemType", "ItemPackage", "ItemPrice", "Quantity", "ItemCareType", "ItemFrequency", "ItemPatCat", "AuditUserID", TO_CHAR("ValidityFrom", 'yyyy-MM-dd')ValidityFrom, TO_CHAR("ValidityTo", 'yyyy-MM-dd')ValidityTo FROM "tblItems" WHERE "ValidityTo" IS NULL"""
        if connection.vendor == "postgresql" else
        """SELECT "ItemID", "ItemUUID", "ItemCode", "ItemName", "ItemType", "ItemPackage", "ItemPrice", "Quantity", "ItemCareType", "ItemFrequency", "ItemPatCat", "AuditUserID", FORMAT("ValidityFrom", 'yyyy-MM-dd')ValidityFrom, FORMAT("ValidityTo", 'yyyy-MM-dd')ValidityTo FROM "tblItems" WHERE "ValidityTo" IS NULL""",

        "subitems": """SELECT "idPCP", "ItemID", "ServiceID", "qty", "price", "status" FROM "tblProductContainedPackage" """,
    }
